...
try {
  ...
  const nodes = await client.query(`SELECT id FROM public.application_nodes WHERE node_key = ANY($1::text[])`, [['ativos-manutencao', ...children.map(item => item[0]), 'importacoes-ativos-empilhadeiras', 'importacoes-ativos-equipamentos-industria', 'importacoes-ativos-ferramentas']]);

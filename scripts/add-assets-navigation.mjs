...
try {
  ...
  const children = [['ativos-empilhadeiras', 'Manutenção · Empilhadeiras', 10], ['ativos-equipamentos-industria', 'Manutenção · Equipamentos da indústria', 20], ['ativos-ferramentas', 'Manutenção · Ferramentas de oficinas e indústria', 30]];
  const catalogRoot = await client.query(`SELECT id FROM public.application_nodes WHERE node_key = 'cadastros' AND active = true`);
  if (!catalogRoot.rows[0]) throw new Error('Raiz Cadastros não encontrada.');
  const catalogNodes = [['cadastros-ativos-empilhadeiras', 'Empilhadeiras', 70], ['cadastros-ativos-equipamentos-industria', 'Equipamentos da indústria', 80], ['cadastros-ativos-ferramentas', 'Ferramentas de oficinas e indústria', 90]];
  for (const [nodeKey, label, sortOrder] of catalogNodes) await client.query(`INSERT INTO public.application_nodes (node_key, label, parent_id, sort_order, active) VALUES ($1, $2, $3, $4, true) ON CONFLICT (node_key) DO UPDATE SET label = EXCLUDED.label, parent_id = EXCLUDED.parent_id, active = true`, [nodeKey, label, catalogRoot.rows[0].id, sortOrder]);
  ...
  const profiles = await client.query(`SELECT id, profile_key FROM public.access_profiles WHERE active = true AND profile_key IN ('development-admin', 'admin', 'administrator', 'operations-admin')`);
  const nodes = await client.query(`SELECT id FROM public.application_nodes WHERE node_key = ANY($1::text[])`, [['ativos-manutencao', ...children.map(item => item[0]), ...catalogNodes.map(item => item[0]), 'importacoes-ativos-empilhadeiras', 'importacoes-ativos-equipamentos-industria', 'importacoes-ativos-ferramentas']]);
  ...
  console.log(JSON.stringify({ rootId, catalogRootId: catalogRoot.rows[0].id, profiles: profiles.rows.map(row => row.profile_key), nodes: nodes.rows.length }));

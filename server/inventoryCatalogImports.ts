...
export async function importCatalogEntries(input: { entity: CatalogImportEntity; rows: CatalogImportRow[] }, identity: PortalIdentity) {
  ...
  try {
    ...
    for (let index = 0; index < input.rows.length; index += 1) {
      ...
      if (input.entity === "costCenter") { const unitCode = String(row.codigo_unidade ?? "").trim(); const unitId = unitCode ? await findId(client, "org_units", "code = $1", [unitCode], "Unidade", line) : null; await client.query("insert into public.cost_centers (code, name, unit_id, active) values ($1, $2, $3, $4) on conflict (code) do update set name = excluded.name, unit_id = excluded.unit_id, active = excluded.active, updated_at = now()", [code, name, unitId, active]); }

...
export const cadastrosRouter = router({
  ...
  saveDirect: publicProcedure.input(z.object({ type: typeSchema, row: z.record(z.string(), z.string()) })).mutation(async ({ ctx, input }) => { const identity = await getPortalIdentity(authorizationHeader(ctx.req.headers)); await assertApplicationPermission(identity, nodeForRegistration[input.type], "manage"); await assertRegistrationOperation(identity, input.type, "create"); return commitRegistrationImport(input.type, [input.row], identity); }),
  saveManaged: publicProcedure.input(z.object({ type: typeSchema, row: z.record(z.string(), z.string()) })).mutation(async ({ ctx, input }) => { const identity = await getPortalIdentity(authorizationHeader(ctx.req.headers)); await assertApplicationPermission(identity, nodeForRegistration[input.type], "manage"); await assertRegistrationOperation(identity, input.type, "manage"); return commitRegistrationImport(input.type, [input.row], identity); }),
  setActive: publicProcedure.input(z.object({ type: typeSchema, code: z.string().trim().min(1).max(320), active: z.boolean() })).mutation(async ({ ctx, input }) => { const identity = await getPortalIdentity(authorizationHeader(ctx.req.headers)); await assertApplicationPermission(identity, nodeForRegistration[input.type], "manage"); await assertRegistrationOperation(identity, input.type, "manage"); return setRegistrationRecordActive(input.type, input.code, input.active, identity); }),
});

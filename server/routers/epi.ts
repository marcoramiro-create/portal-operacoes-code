...
async function audit(identity: PortalIdentity, entityType: string, entityId: string, action: string, details: Record<string, unknown> = {}) {
  await getSupabasePool().query(
    ...
    [identity.userId, entityType, entityId, action, JSON.stringify(details)]
  );
}
...
export const epiRouter = router({
  listCertificates: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      const identity = await PortalIdentity.fromContext(ctx);
      await assertApplicationPermission(identity, "cadastros-epis", "view");
      const pool = getSupabasePool();
      const params: string[] = [];
      let query = `
        select ec.id, ec.product_id, p.product_code, p.name as product_name,
               ec.ca_number, ec.manufacturer, ec.ca_issued_at, ec.ca_expires_at,
               ec.status, ec.created_at, ec.updated_at
        from public.epi_certificates ec
  ...
  createCertificate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const result = await pool.query(
        `insert into public.epi_certificates (product_id, ca_number, manufacturer, ca_issued_at, ca_expires_at)
  ...
  deactivateCertificate: protectedProcedure
  ...
  listDeliveries: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const params: string[] = [];
      let query = `
        select ed.id, ed.product_id, p.product_code, p.name as product_name,
               ed.employee_id, e.full_name as employee_name,
               ed.ca_number, ed.batch, ed.size, ed.quantity,
               ed.delivered_at, ed.expected_return_at, ed.returned_at, ed.returned_quantity,
               ed.status, ed.notes, ed.created_at
        from public.epi_deliveries ed
        join public.products p on p.id = ed.product_id
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const result = await pool.query(
        `insert into public.epi_deliveries
          (product_id, employee_id, ca_number, batch, size, quantity, delivered_at, expected_return_at, notes, delivered_by_user_id)
        ...
        [input.productId, input.employeeId, input.caNumber || null, input.batch || null,
         ...
         input.expectedReturnAt || null, input.notes || null, identity.userId]
      );
      await audit(identity, "epi_delivery", result.rows[0].id, "created", input);
      return { id: result.rows[0].id };
    }),

  returnDelivery: protectedProcedure
    .input(z.object({
      id: z.string().uuid(),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await pool.query(
        `update public.epi_deliveries
         set returned_at = current_date, returned_quantity = $2, status = 'returned', notes = $3, updated_at = now()
         where id = $1`,
  ...
  caAlerts: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const result = await pool.query(
        `select ec.id, ec.product_id, p.product_code, p.name as product_name,
                ec.ca_number, ec.manufacturer, ec.ca_expires_at, ec.status,
                case
                  when ec.status = 'revoked' then 'revogado'
                  when ec.ca_expires_at is not null and ec.ca_expires_at < current_date then 'vencido'
                  when ec.ca_expires_at is not null and ec.ca_expires_at <= current_date + interval '${days} days' then 'vencendo'
                  else 'vigente'
                end as alert_status,
                ec.ca_expires_at - current_date as days_remaining
         from public.epi_certificates ec
         join public.products p on p.id = ec.product_id
         where ec.status = 'active'
           and (ec.ca_expires_at is null or ec.ca_expires_at <= current_date + interval '${days} days')
         order by ec.ca_expires_at asc nulls last`,
        []
      );
      return result.rows;
    }),

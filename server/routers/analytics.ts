...
export const analyticsRouter = router({
  ...
  cardItems: publicProcedure.input(z.object({ page: z.number().int().min(1).default(1), sort: z.enum(["code", "branch", "turnover", "curve", "stock", "stockValue", "pedidos", "coverageDays", "consumoMensal", "productType"]).default("stockValue"), dir: z.enum(["asc", "desc"]).default("desc"), metric: z.enum(["lowCoverage", "stockValue", "excess", "withoutSales"]), ...branchFilterSchema, curve: curveSchema.optional(), productType: z.enum(["ME", "PE"]).optional(), mrp: z.enum(["Sim", "Não"]).optional(), family: z.string().min(1).optional(), subfamily: z.string().min(1).optional() })).query(async ({ ctx, input }) => { await modulePermission(ctx, "view"); const { metric, page, sort, dir, ...filters } = input; return getAnalyticsCardItems(filters satisfies AnalyticsFilter, metric, page, 200, sort, dir); }),

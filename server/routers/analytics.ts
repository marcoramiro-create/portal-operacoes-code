...
import { assertApplicationPermission, assertPortalAdministrator, getPortalIdentity, recordPortalAudit } from "../supabasePortal";
...
export const analyticsRouter = router({
  ...
  canAdminister: publicProcedure.query(async ({ ctx }) => { const identity = await modulePermission(ctx, "view"); return identity.isDevelopmentAdmin || identity.profiles.includes("operations-admin"); }),

...
import { commitAgra045Warehouses, commitSi3CostCenters, previewAgra045Warehouses, previewSi3CostCenters } from "../protheusImportPreviews";
...
const catalogEntryUpdateSchema = z.discriminatedUnion("entity", [
  ...
  z.object({ entity: z.literal("costCenter"), id: z.string().uuid(), code: text, name: text, unitId: z.string().uuid().optional(), branchId: z.string().uuid() }),
...
export const inventoryCatalogRouter = router({
  ...
  createCostCenter: publicProcedure.input(z.object({ code: text, name: text, unitId: z.string().uuid().optional(), branchId: z.string().uuid() })).mutation(async ({ ctx, input }) => createCostCenter(input, await getPortalIdentity(authorizationHeader(ctx.req.headers)))),
  ...
  importEntries: publicProcedure.input(z.object({ entity: z.enum(["productType", "orgUnit", "costCenter", "company", "branch", "warehouse", "stockLocation"]), rows: z.array(z.record(z.string(), z.string())).min(1).max(500), centralized: z.boolean().optional() })).mutation(async ({ ctx, input }) => importCatalogEntries(input, await getPortalIdentity(authorizationHeader(ctx.req.headers)))),
  previewAgra045: publicProcedure.input(z.object({ content: z.string().min(1).max(1_000_000), centralized: z.boolean().optional() })).mutation(async ({ ctx, input }) => { const identity = await getPortalIdentity(authorizationHeader(ctx.req.headers)); return previewAgra045Warehouses(input.content, identity, input.centralized ? "importacoes-armazens" : undefined); }),
  commitAgra045: publicProcedure.input(z.object({ content: z.string().min(1).max(1_000_000), centralized: z.boolean().optional() })).mutation(async ({ ctx, input }) => commitAgra045Warehouses(input.content, await getPortalIdentity(authorizationHeader(ctx.req.headers)), input.centralized ? "importacoes-armazens" : undefined)),
  previewSi3: publicProcedure.input(z.object({ content: z.string().min(1).max(2_000_000), centralized: z.boolean().optional() })).mutation(async ({ ctx, input }) => { const identity = await getPortalIdentity(authorizationHeader(ctx.req.headers)); if (input.centralized) await assertApplicationPermission(identity, "importacoes-centros-custo", "manage"); return previewSi3CostCenters(input.content); }),
  commitSi3: publicProcedure.input(z.object({ content: z.string().min(1).max(2_000_000), centralized: z.boolean().optional() })).mutation(async ({ ctx, input }) => commitSi3CostCenters(input.content, await getPortalIdentity(authorizationHeader(ctx.req.headers)), input.centralized ? "importacoes-centros-custo" : undefined)),

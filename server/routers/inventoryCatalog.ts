...
export const inventoryCatalogRouter = router({
  ...
  previewSi3: publicProcedure.input(z.object({ content: z.string().min(1).max(2_000_000) })).mutation(async ({ ctx, input }) => previewSi3CostCenters(input.content)),
  commitSi3: publicProcedure.input(z.object({ content: z.string().min(1).max(2_000_000) })).mutation(async ({ ctx, input }) => commitSi3CostCenters(input.content, await getPortalIdentity(authorizationHeader(ctx.req.headers)))),

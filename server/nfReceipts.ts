...
// Pontos fixos: Descarga / Recebimento / Conferência / Envio ao fiscal.
// O ponto nunca fica travado no usuário (suporta rotação).
// ============================================================
...
type NfReceiptRow = { id: string; access_key: string; issuer_cnpj: string; invoice_model: string; invoice_series: string; invoice_number: string; issued_year_month: string; capture_method: CaptureMethod; reading_point: ReadingPoint; captured_at: Date; captured_by: string | null; protheus_sc7_reference: string | null; nf_legal_reference: string | null; matched_at: Date | null; supplier_code: string | null; supplier_store: string | null; supplier_legal_name: string | null; supplier_trade_name: string | null };
function mapNfReceiptRow(row: NfReceiptRow) {
  return { id: row.id, accessKey: row.access_key, issuerCnpj: row.issuer_cnpj, invoiceModel: row.invoice_model, invoiceSeries: row.invoice_series, invoiceNumber: row.invoice_number, issuedYearMonth: row.issued_year_month, captureMethod: row.capture_method, readingPoint: row.reading_point, capturedAt: row.captured_at, capturedBy: row.captured_by, protheusSc7Reference: row.protheus_sc7_reference, nfLegalReference: row.nf_legal_reference, matchedAt: row.matched_at, supplier: row.supplier_code ? { code: row.supplier_code, store: row.supplier_store, legalName: row.supplier_legal_name, tradeName: row.supplier_trade_name } : null };
}
...
export async function createNfReceipt(input: { accessKey: string; captureMethod: CaptureMethod; readingPoint: ReadingPoint }, identity: PortalIdentity) {
  await assertApplicationPermission(identity, "chaves-nf", "manage");
  if (!isReadingPoint(input.readingPoint)) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "Informe o ponto de leitura: Descarga, Recebimento, Conferência ou Envio ao fiscal." });
  }
  const parsed = parseNfAccessKey(input.accessKey);
  try {
    const result = await getSupabasePool().query<{ id: string; captured_at: Date }>(
      `insert into public.nf_receipts (access_key, issuer_cnpj, invoice_model, invoice_series, invoice_number, issued_year_month, capture_method, reading_point, captured_by_user_id)
       values ($1, $2, $3, $4, $5, $6, $7, $8, $9) returning id, captured_at`,
      [parsed.accessKey, parsed.issuerCnpj, parsed.invoiceModel, parsed.invoiceSeries, parsed.invoiceNumber, parsed.issuedYearMonth, input.captureMethod, input.readingPoint, identity.id],
    );

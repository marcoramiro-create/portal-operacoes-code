...
export async function listAssets(type: AssetType, identity: PortalIdentity) {
  ...
  const result = await getPool().query(`SELECT id, assetType, code, name, serialNumber, externalIdentifier, status, criticality, sector, productionLine, physicalPosition, calibrationRequired, calibrationDueAt, metadata, createdAt, updatedAt FROM assetAssets WHERE assetType = $1 AND active = 1 ORDER BY code`, [type]);
  return result.rows;
}
...
export async function createAsset(input: AssetInput, identity: PortalIdentity) {
  ...
  try {
    ...
    await getPool().query(`INSERT INTO assetEvents (id, assetId, eventType, details, actorUserId) VALUES ($1, $2, 'created', $3, $4)`, [randomUUID(), id, JSON.stringify({ code: input.code, assetType: input.assetType }), identity.id]);
    return { id };
  } catch (error: any) {
...
export async function createMaintenance(input: MaintenanceInput, type: AssetType, identity: PortalIdentity) {
  ...
  await getPool().query(`INSERT INTO assetEvents (id, assetId, eventType, details, actorUserId) VALUES ($1, $2, 'maintenance_requested', $3, $4)`, [randomUUID(), input.assetId, JSON.stringify({ maintenanceId: id, maintenanceType: input.maintenanceType }), identity.id]);
  return { id };
}

export async function approveMaintenance(id: string, identity: PortalIdentity) {
  const result = await getPool().query(`SELECT o.id, a.assetType, o.assetId FROM assetMaintenanceOrders o JOIN assetAssets a ON a.id = o.assetId WHERE o.id = $1`, [id]);
...
export async function updateMaintenanceStatus(id: string, status: "in_progress" | "waiting_parts" | "completed" | "cancelled", identity: PortalIdentity) {
  ...
  if (timestampField) {
    await getPool().query(`UPDATE assetMaintenanceOrders SET status = $1, "${timestampField}" = CURRENT_TIMESTAMP, updatedAt = CURRENT_TIMESTAMP WHERE id = $2`, [status, id]);
  } else {
    await getPool().query(`UPDATE assetMaintenanceOrders SET status = $1, updatedAt = CURRENT_TIMESTAMP WHERE id = $2`, [status, id]);
  }
  await getPool().query(`INSERT INTO assetEvents (id, assetId, eventType, details, actorUserId) VALUES ($1, $2, $3, $4, $5)`, [randomUUID(), current.assetId, `maintenance_${status}`, JSON.stringify({ maintenanceId: id }), identity.id]);
  return { success: true };
}
...
export async function assetSummary(type: AssetType, identity: PortalIdentity) {
  await permission(identity, type, "view");
  const result = await getPool().query(`SELECT status, COUNT(*) AS total FROM assetAssets WHERE assetType = $1 AND active = 1 GROUP BY status ORDER BY status`, [type]);

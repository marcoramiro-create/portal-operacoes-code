...
export async function listInventoryCatalog(identity: PortalIdentity) {
  ...
  const [productTypes, companies, branches, warehouses, stockLocations, products] = await Promise.all([
    ...
    database.query<{ id: string; product_code: string; name: string; product_type_id: string | null; product_type_code: string | null; unit_of_measure: string; requires_size: boolean; requires_lot: boolean; requires_expiration: boolean; requires_ca: boolean; active: boolean }>("select product.id, product.product_code, product.name, product.product_type_id, type.code as product_type_code, product.unit_of_measure, product.requires_size, product.requires_lot, product.requires_expiration, product.requires_ca, product.active from public.products product left join public.product_types type on type.id = product.product_type_id order by product.product_code"),
  ]);
  return {
    ...
    products: products.rows.map(row => ({ id: row.id, code: row.product_code, name: row.name, productTypeId: row.product_type_id, productTypeCode: row.product_type_code, unitOfMeasure: row.unit_of_measure, requiresSize: row.requires_size, requiresLot: row.requires_lot, requiresExpiration: row.requires_expiration, requiresCa: row.requires_ca, active: row.active })),
  };
}
...
export async function configureInventoryProduct(input: { productId: string; productTypeId: string; unitOfMeasure: string; requiresSize: boolean; requiresLot: boolean; requiresExpiration: boolean; requiresCa: boolean }, identity: PortalIdentity) {
  await assertCatalogManagement(identity);
  const unitOfMeasure = trimRequired(input.unitOfMeasure, "Unidade de medida").toUpperCase();
  const result = await getSupabasePool().query<{ id: string }>("update public.products set product_type_id = $2, unit_of_measure = $3, requires_size = $4, requires_lot = $5, requires_expiration = $6, requires_ca = $7, updated_at = now() where id = $1 returning id", [input.productId, input.productTypeId, unitOfMeasure, input.requiresSize, input.requiresLot, input.requiresExpiration, input.requiresCa]);
  if (!result.rows[0]) throw new TRPCError({ code: "NOT_FOUND", message: "Produto não encontrado." });
  await audit(identity, "product", input.productId, "inventory_configured", { productTypeId: input.productTypeId, unitOfMeasure, requiresSize: input.requiresSize, requiresLot: input.requiresLot, requiresExpiration: input.requiresExpiration, requiresCa: input.requiresCa });

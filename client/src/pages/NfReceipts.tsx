    ...
    <div className="page-wrap">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.06fr_.94fr]">
        ...
        {activeView === "history" && <section className="sc-surface overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-5 sm:px-7"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#d8ebfa] text-slate-950"><ShieldCheck className="h-5 w-5" /></span><div><h2 className="text-lg font-extrabold tracking-tight text-slate-950">Últimas leituras</h2><p className="mt-0.5 text-xs font-medium text-slate-500">Registro auditável com usuário, ponto, data, hora e dados da descarga.</p></div></div><Button size="sm" variant="outline" onClick={() => void exportReadings()} disabled={exportRows.isFetching}><Download className="mr-2 h-4 w-4" />{exportRows.isFetching ? "Preparando…" : "Exportar Excel"}</Button></div>
        ...
        {activeView === "carriers" && <section className="sc-surface p-5 sm:p-7">
          <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#d9f0e3] text-slate-950"><Truck className="h-5 w-5" /></span><div><h2 className="text-lg font-extrabold tracking-tight text-slate-950">Transportadoras (SA4)</h2><p className="mt-0.5 text-xs font-medium text-slate-500">Importe o cadastro de transportadoras para facilitar a seleção na Descarga. O CNPJ é a chave (evita duplicidade) e a operação nunca é restritiva.</p></div></div>
          <div className="mt-6">

...
// MUDANÇA (07/09/2026): adiciona importação de SB1, SBZ, Famílias e SubFamílias.
// ============================================================
...
// Converte um arquivo selecionado para base64 (usado no envio ao servidor).
...
type ReferenceImporterProps = {
  ...
  description: string;
  mutation: {
    mutate: (input: { contentBase64: string }) => void;
    isPending: boolean;
    isSuccess: boolean;
  };
};
...
function ReferenceImporter({ title, description, mutation }: ReferenceImporterProps) {
  const [file, setFile] = useState<File | null>(null);

  // Limpa o arquivo selecionado depois que a importação termina com sucesso.
  useEffect(() => {
    if (mutation.isSuccess) setFile(null);
  }, [mutation.isSuccess]);

  // Valida o arquivo selecionado (.xlsx, até 18 MB).
  const chooseFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected && selected.size > 18 * 1024 * 1024) {
      toast.error("O arquivo deve ter no máximo 18 MB.");
      event.target.value = "";
      return;
    }
    setFile(selected);
  ...
  // Envia o arquivo para a rota de importação do cadastro.
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!file) return toast.error(`Selecione o arquivo ${title}.`);
    mutation.mutate({ contentBase64: await toBase64(file) });
  };
    ...
    <form onSubmit={submit} className="flex flex-col gap-3 border-t border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
      ...
      <div className="flex shrink-0 items-center gap-2">
        <label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 shadow-sm transition-colors hover:border-slate-400">
          ...
          <span>{file ? "Trocar arquivo" : "Selecionar"}</span>
        ...
        <Button type="submit" size="sm" disabled={!file || mutation.isPending} className="bg-slate-950 text-white hover:bg-slate-800">
          {mutation.isPending ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : <Upload className="mr-2 h-3.5 w-3.5" />}
...
export default function ImportData() {
  ...
  // MUDANÇA (07/09/2026): mutações para importar os cadastros de referência (SB1, SBZ, Famílias, SubFamílias).
  const invalidateAnalytics = async () => {
    await Promise.all([utils.analytics.imports.invalidate(), utils.analytics.dashboard.invalidate(), utils.analytics.filterOptions.invalidate()]);
  };
  const importSb1 = trpc.analytics.importReferenceSb1.useMutation({
    onSuccess: async result => { await invalidateAnalytics(); toast.success(`SB1 importado: ${result.count.toLocaleString("pt-BR")} itens de referência.`); },
    onError: error => toast.error(error.message),
  });
  const importSbz = trpc.analytics.importReferenceSbz.useMutation({
    onSuccess: async result => { await invalidateAnalytics(); toast.success(`SBZ importado: ${result.count.toLocaleString("pt-BR")} itens de referência.`); },
    onError: error => toast.error(error.message),
  });
  const importFamilias = trpc.analytics.importReferenceFamilias.useMutation({
    onSuccess: async result => { await invalidateAnalytics(); toast.success(`Famílias importadas: ${result.count.toLocaleString("pt-BR")} registros.`); },
    onError: error => toast.error(error.message),
  });
  const importSubFamilias = trpc.analytics.importReferenceSubFamilias.useMutation({
    onSuccess: async result => { await invalidateAnalytics(); toast.success(`SubFamílias importadas: ${result.count.toLocaleString("pt-BR")} registros.`); },
    onError: error => toast.error(error.message),
  });
    ...
    {canAdminister && (
      <section className="sc-surface mt-5 overflow-hidden">
        <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-5 sm:px-7">
          ...
          <div>
            <h2 className="text-base font-extrabold tracking-tight text-slate-950">Cadastros de referência</h2>
            <p className="text-xs font-medium text-slate-500">Importe SB1, SBZ, Famílias e SubFamílias para cruzar Tipo (ME/PE), MRP, família e subfamília com os itens das cargas.</p>
          </div>
        </div>
        <ReferenceImporter title="SB1 · Cadastro de produtos" description="Fornece Tipo (ME/PE), Família e Subfamília de cada item." mutation={importSb1} />
        <ReferenceImporter title="SBZ · Cadastro por filial" description="Fornece MRP (Sim/Não) e estoques mínimo/máximo por filial." mutation={importSbz} />
        <ReferenceImporter title="Famílias" description="Nomes das famílias (cabeçalho na linha 4)." mutation={importFamilias} />
        <ReferenceImporter title="SubFamílias" description="Nomes das subfamílias (cabeçalho na linha 4)." mutation={importSubFamilias} />
      </section>
    )}

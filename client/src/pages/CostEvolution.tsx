...
export function CostEvolutionDashboard({ segment }: { segment: Segment }) {
  ...
  // Na Indústria, NÃO filtramos por filial: a análise traz todos os itens da base de uma vez.
  // (A filial 0105 é exibida como referência, mas não entra na consulta — evita retornar vazio
  //  caso a coluna de filial da planilha da indústria não contenha exatamente "0105".)
  const input = useMemo(() => ({
    segment,
    periodoInicio: applied.periodoInicio || undefined,
    periodoFim: applied.periodoFim || undefined,
    filial: isIndustry ? undefined : (applied.filial || undefined),
  ...
  const applyFilters = () => {
    setApplied({ filial: isIndustry ? "" : filial, codAgregado, descricao, searchType, periodoInicio, periodoFim });
    setRunAnalysis(true);
  ...
  return <div className="page-wrap">
    ...
    <section className="sc-surface p-5 sm:p-7">
      {isIndustry && <div className="mb-4 rounded-xl bg-sky-50 px-4 py-3 text-xs font-semibold text-sky-800">Segmento <strong>Indústria</strong>: a análise usa sempre a filial única <strong>0105</strong> — o filtro de
...
import { AlertTriangle, ChartNoAxesCombined, ChevronDown, ChevronRight, ChevronUp, FileUp, Lightbulb, Sparkles, Store, Tags, X } from "lucide-react";
  ...
  return <div className="mx-auto max-w-[1440px]">
    ...
    {!hasImport && !isLoading ? (
      ...
      {dashboardView === "overview" && <>
        <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
          <KpiCard label="Cobertura consolidada" value={formatNumber(kpis.coverage, 1)} unit="dias" hint="Σ estoque ÷ Σ consumo diário" />
          <KpiCard label="Baixa cobertura" value={formatMoney(data?.quality.lowCoverageStockValue ?? 0)} hint="Estoque em itens com cobertura < 30 dias" />
          <KpiCard label="Estoque financeiro" value={formatMoney(kpis.stockValue)} hint="Valor em estoque da seleção" />
          <KpiCard label="Vendas acumuladas" value={formatMoney(kpis.salesValue)} hint="Vendas dos 13 meses da carga" />
          <KpiCard label="Excedente" value={formatMoney(kpis.excessValue)} hint="Acima do alvo da curva" />
          <KpiCard label="Giro" value={formatNumber(kpis.turnover, 2)} unit="vezes" hint="Vendas ÷ estoque" />
        </section>
        ...
        {drill.branch && drill.family && <section className="sc-surface mt-5 overflow-hidden"><div className="flex flex-col gap-2 border-b border-slate-100 px-5 py-5 sm:px-7"><div><p className="eyebrow">Itens</p><h2 className="mt-2 text-base font-extrabold tracking-tight text-slate-950">Itens da família “{drill.family}” na unidade {drill.branch}</h2></div><p className="text-xs font-semibold text-slate-500">{(itemPage?.items ?? []).length} itens nesta página · {itemPage?.total ?? 0} no total da família.</p></div><Table><TableHeader><TableRow><TableHead>Item</TableHead><TableHead>Unidade</TableHead><TableHead>Curva</TableHead><TableHead>Giro</TableHead><TableHead>Cobertura</TableHead><TableHead>Estoque</TableHead></TableRow></TableHeader><TableBody>{(itemPage?.items ?? []).length === 0 && <TableRow><TableCell colSpan={6} className="h-24 text-center text-sm font-medium text-slate-500">Nenhum item encontrado para a família selecionada.</TableCell></TableRow>}{(itemPage?.items ?? []).map(item => <TableRow key={item.id}><TableCell><p className="font-bold text-slate-950">{item.code}</p><p className="max-w-72 truncate text-xs text-slate-500">{item.description}</p></TableCell><TableCell>{item.branch}</TableCell><TableCell>{item.curve}</TableCell><TableCell>{formatNumber(item.turnover, 2)}</TableCell><TableCell>{formatNumber(item.coverageDays, 1)} dias</TableCell><TableCell>{formatMoney(item.stockValue)}</TableCell></TableRow>)}</TableBody></Table><div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 px-5 py-4 text-xs font-bold text-slate-600 sm:px-7"><span>página {itemPage?.page ?? itemsPage} de {totalPages}</span><div className="flex gap-2"><Button size="sm" variant="outline" disabled={(itemPage?.page ?? itemsPage) <= 1} onClick={() => setItemsPage(page => page - 1)}>Anterior</Button><Button size="sm" variant="outline" disabled={(itemPage?.page ?? itemsPage) >= totalPages} onClick={() => setItemsPage(page => page + 1)}>Próxima</Button></div></div></section>}
      </>}

      {dashboardView === "analysis" && <>
        {/* P2 (09/09/2026): comparação lado a lado de cargas */}
        {comparableImports.length > 0 && <section className="sc-surface mt-5 overflow-hidden"><div className="flex flex-col gap-2 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-end sm:justify-between sm:px-7"><div><p className="eyebrow">Comparação de cargas</p><h2 className="mt-2 text-base font-extrabold tracking-tight text-slate-950">Atual vs. versão anterior</h2></div><p className="max-w-xl text-xs font-semibold leading-5 text-slate-500">Mesmos filtros aplicados sobre duas cargas aprovadas, com variação percentual.</p></div><div className="px-5 py-4 sm:px-7"><label className="grid max-w-md gap-2"><span className="text-xs font-extrabold uppercase tracking-[0.1em] text-slate-500">Comparar com</span><select className={selectClass} value={compareImportId ?? ""} onChange={event => setCompareImportId(event.target.value ? Number(event.target.value) : null)}><option value="">Selecione uma versão anterior…</option>{comparableImports.map(imp => <option key={imp.id} value={imp.id}>{imp.fileName} · {formatDate(imp.importedAt)}</option>)}</select></label>{compareImportId && comparedImport && comparison && <Table className="mt-4"><TableHeader><TableRow><TableHead>Indicador</TableHead><TableHead>Carga atual</TableHead><TableHead>{comparedImport.fileName}</TableHead><TableHead>Variação</TableHead></TableRow></TableHeader><TableBody>{comparisonRows.map(row => { const pct = row.comparado === 0 ? (row.atual === 0 ? 0 : 100) : ((row.atual - row.comparado) / Math.abs(row.comparado)) * 100; const good = row.invert ? pct <= 0 : pct >= 0; return <TableRow key={row.metric}><TableCell className="font-bold text-slate-950">{row.metric}</TableCell><TableCell>{row.format(row.atual)}</TableCell><TableCell>{row.format(row.comparado)}</TableCell><TableCell><span className={`font-extrabold ${pct === 0 ? "text-slate-400" : good ? "text-emerald-600" : "text-rose-600"}`}>{pct > 0 ? "▲" : pct < 0 ? "▼" : "•"} {formatNumber(Math.abs(pct), 1)}%</span></TableCell></TableRow>; })}</TableBody></Table>}</div></section>}
...
function AnalyticsTable({ title, groupLabel, icon: Icon, rows, onRowClick, clickableHint }: { title: string; groupLabel: string; icon: typeof Store; rows: Array<{ label: string; salesValue13M: number; stockValue: number; turnover: number; coverageDays: number; excessValue: number }>; onRowClick?: (label: string) => void; clickableHint?: string }) { return <section className="sc-surface overflow-hidden"><div className="flex items-center gap-3 border-b border-slate-100 px-5 py-5 sm:px-7"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#dcebf7] text-slate-950"><Icon className="h-4 w-4" /></span><div><h2 className="text-base font-extrabold tracking-tight text-slate-950">{title}</h2>{clickableHint ? <p className="text-[11px] font-semibold text-slate-400">{clickableHint}</p> : null}</div></div><Table><TableHeader><TableRow><TableHead>{groupLabel}</TableHead><TableHead>Giro</TableHead><TableHead>Vendas</TableHead><TableHead>Estoque</TableHead><TableHead>Cobertura</TableHead><TableHead>Excedente</TableHead></TableRow></TableHeader><TableBody>{rows.length === 0 && <TableRow><TableCell colSpan={6} className="h-28 text-center text-sm font-medium text-slate-500">Nenhum dado encontrado para os filtros selecionados.</TableCell></TableRow>}{rows.map(row => <TableRow key={row.label} className={onRowClick ? "cursor-pointer hover:bg-slate-50" : undefined} onClick={onRowClick ? () => onRowClick(row.label) : undefined}><TableCell className="max-w-48 font-bold text-slate-950">{row.label}{onRowClick ? <ChevronRight className="ml-1 inline h-3.5 w-3.5 text-slate-400" /> : null}</TableCell><TableCell>{formatNumber(row.turnover, 2)}</TableCell><TableCell>{formatMoney(row.salesValue13M)}</TableCell><TableCell>{formatMoney(row.stockValue)}</TableCell><TableCell>{formatNumber(row.coverageDays, 1)} dias</TableCell><TableCell>{formatMoney(row.excessValue)}</TableCell></TableRow>)}</TableBody></Table></section>; }
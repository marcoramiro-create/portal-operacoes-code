...
type ItemRow = { id: string; code: string; description: string; branch: string; curve: string; turnover: number; coverageDays: number; stock: number; stockValue: number; ultimaCompra: string | null; productType: string };
...
export default function Dashboard() {
  ...
  const cardInput = useMemo(() => cardMetric ? ({ metric: cardMetric, branches: input.branches, curve: input.curve, productType: input.productType, mrp: input.mrp, family: input.family, subfamily: input.subfamily }) : undefined, [cardMetric, input]);
  ...
  return <div className="mx-auto max-w-[1440px]">
    ...
    {!hasImport && !isLoading ? (
      ...
      {dashboardView === "overview" && <>
        ...
        {cardMetric && <section className="sc-surface mt-5 overflow-hidden"><div className="flex flex-col gap-2 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-end sm:justify-between sm:px-7"><div><p className="eyebrow">Detalhamento do indicador</p><h2 className="mt-2 text-base font-extrabold tracking-tight text-slate-950">{cardMetric === "lowCoverage" ? "Baixa cobertura" : cardMetric === "stockValue" ? "Estoque financeiro" : cardMetric === "excess" ? "Excedente" : "Capital parado sem vendas"}</h2></div><div className="flex items-center gap-3"><p className="text-xs font-semibold text-slate-500">{isCardLoading ? "Carregando…" : `${cardItems.length} itens`}</p><Button size="sm" variant="outline" onClick={() => setCardMetric(null)}>Fechar</Button></div></div><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Produto</TableHead><TableHead>Filial</TableHead><TableHead className="text-right">Giro</TableHead><TableHead>Data da última compra</TableHead><TableHead>Curva</TableHead><TableHead className="text-right">Estoque quantitativo</TableHead><TableHead className="text-right">Estoque financeiro</TableHead><TableHead>Tipo de produto</TableHead></TableRow></TableHeader><TableBody>{!isCardLoading && cardItems.length === 0 && <TableRow><TableCell colSpan={8} className="h-24 text-center text-sm font-medium text-slate-500">Nenhum item encontrado para este indicador.</TableCell></TableRow>}{cardItems.map(item => <TableRow key={item.id}><TableCell><p className="font-bold text-slate-950">{item.code}</p><p className="max-w-72 truncate text-xs text-slate-500">{item.description}</p></TableCell><TableCell>{item.branch}</TableCell><TableCell className="text-right">{formatNumber(item.turnover, 2)}</TableCell><TableCell>{item.ultimaCompra ? new Date(`${item.ultimaCompra}T00:00:00`).toLocaleDateString("pt-BR") : "—"}</TableCell><TableCell>{item.curve}</TableCell><TableCell className="text-right">{formatNumber(item.stock, 3)}</TableCell><TableCell className="text-right">{formatMoney(item.stockValue)}</TableCell><TableCell>{item.productType}</TableCell></TableRow>)}</TableBody></Table></div></section>}

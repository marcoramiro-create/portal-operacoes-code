...
type ItemRow = { id: string; code: string; description: string; branch: string; curve: string; turnover: number; coverageDays: number; stockValue: number };
  ...
  return <div className="mx-auto max-w-[1440px]">
    ...
    {!hasImport && !isLoading ? (
      ...
      {dashboardView === "overview" && <>
        ...
        {/* P3 (10/09/2026): 6 KPIs + cards de qualidade unificados num único bloco responsivo */}
        <section id="metrics-anchor" className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8">
          ...
          <MetricCard label="Baixa cobertura" value={formatMoney(data?.quality.lowCoverageStockValue ?? 0)} hint="Estoque em itens com cobertura < 30 dias" />
          <MetricCard label="Estoque financeiro" value={formatMoney(kpis.stockValue)} hint="Valor em estoque da seleção" />
          <MetricCard label="Vendas acumuladas" value={formatMoney(kpis.salesValue)} hint="Vendas dos 13 meses da carga" />
          <MetricCard label="Excedente" value={formatMoney(kpis.excessValue)} hint="Acima do alvo da curva" />
          <MetricCard label="Giro" value={formatNumber(kpis.turnover, 2)} unit="vezes" hint="Vendas ÷ estoque" />
          <MetricCard label="Capital parado sem vendas" value={formatMoney(data?.quality.stockWithoutSalesValue ?? 0)} hint="Estoque financeiro com vendas acumuladas iguais a zero." emphasis="secondary" />
          <MetricCard label="Excedente financeiro" value={formatMoney(data?.quality.excessStockValue ?? 0)} hint="Excedente identificado na extração mais recente aprovada." emphasis="secondary" />
        </section>
...
function MetricCard({ label, value, unit, hint, emphasis = "primary" }: { label: string; value: string; unit?: string; hint: string; emphasis?: "primary" | "secondary" }) { return <article title={hint} className={`rounded-2xl border p-4 ${emphasis === "primary" ? "border-slate-100 bg-white shadow-sm" : "border-slate-100 bg-slate-50"}`}><p className="text-xs font-extrabold uppercase tracking-[0.1em] text-slate-500">{label}</p><p className={`mt-3 font-extrabold tracking-tight text-slate-950 ${emphasis === "primary" ? "text-2xl" : "text-xl"}`}>{value}{unit ? <span className="ml-1 text-sm font-bold text-slate-400">{unit}</span> : null}</p><p className="mt-2 text-[11px] font-medium leading-4 text-slate-400">{hint}</p></article>; }

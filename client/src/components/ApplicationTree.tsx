...
function TreeNode({ node, depth }: { node: ApplicationTreeNode; depth: number }) {
  ...
  const handleClick = () => { if (path) setLocation(path); else if (hasChildren) setExpanded(value => !value); };
  return <div><button type="button" onClick={handleClick} className={`flex min-h-8 w-full items-center gap-2 rounded-lg py-1.5 pr-2 text-left text-[12px] font-semibold transition-colors ${active ? "bg-white text-slate-950 shadow-sm" : path || hasChildren ? "text-slate-700 hover:bg-white/75" : "cursor-default text-slate-500"}`} style={{ paddingLeft: `${12 + depth * 13}px` }} aria-expanded={hasChildren ? expanded : undefined}>{hasChildren ? (expanded ? <ChevronDown className="h-3.5 w-3.5 shrink-0 text-slate-400" /> : <ChevronRight className="h-3.5 w-3.5 shrink-0 text-slate-400" />) : <span className="w-3.5 shrink-0" />}<Icon className={`h-3.5 w-3.5 shrink-0 ${active ? "text-slate-950" : "text-slate-500"}`} /><span className="min-w-0 truncate">{node.label}</span></button>{hasChildren && expanded && <div>{node.children.map(child => <TreeNode key={child.id} node={child} depth={depth + 1} />)}</div>}</div>;
}
